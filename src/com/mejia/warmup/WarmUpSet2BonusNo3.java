package com.mejia.warmup;

import java.util.Arrays;

public class WarmUpSet2BonusNo3 {
    public static void main(String[] args) {
        System.out.println(differenceMaxMin(new int[]{44, 32, 86, 19}));
    }

    public static int differenceMaxMin(int[] arr) {
        Arrays.sort(arr);
        System.out.println("intArray after sorting: " + Arrays.toString(arr));
        int maxNum = arr[arr.length - 1];
        int minNum = arr[0];
        int difference = maxNum - minNum;
        System.out.println("Smallest number is " + minNum + ", biggest number is " + maxNum + ".");
        return difference;
    }
}


//Create a function that takes an array and returns the difference between the biggest and smallest numbers.
//
//Examples
//differenceMaxMin([10, 4, 1, 4, -10, -50, 32, 21]) ➞ 82
//// Smallest number is -50, biggest is 32.
//
//differenceMaxMin([44, 32, 86, 19]) ➞ 67
//// Smallest number is 19, biggest is 86.